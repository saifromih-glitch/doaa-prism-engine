...
memory=KnowledgeMemory(LIB['entries'])
retrieval=memory.retrieve(task, top_k=3)
compact_entries=retrieval['selected_claims']
library_prompt=task+'\n\nوحدات Doaa المنتقاة فقط (استخدمها كقيود ومراجع، ولا توسّع دلالتها):\n'+retrieval['prompt_payload']+'\n\nأخرج خطة قابلة للتنفيذ مع ربط كل قاعدة بمعرف المصدر، واذكر ما يحتاج تحققاً بشرياً.'
...
result={'status':'live_library_pilot_complete','model':MODEL,'task':task,'library_id':LIB['library_id'],'library_entries':len(LIB['entries']),'selected_entries':retrieval['selected_ids'],'retrieval':retrieval,'baseline':baseline,'with_library':with_library,
'prompt_token_delta':with_library['prompt_tokens']-baseline['prompt_tokens'],'total_token_delta':with_library['total_tokens']-baseline['total_tokens'],'prompt_saving_ratio':round((baseline['prompt_tokens']-with_library['prompt_tokens'])/max(1,baseline['prompt_tokens']),6),'total_saving_ratio':round((baseline['total_tokens']-with_library['total_tokens'])/max(1,baseline['total_tokens']),6),'human_quality_review':False,'execution_authority':'none','automatic_promotion':False}
