...
def run(payload):
    ...
    allowed = {("remove_ascii_phone_separators", "phone"), ("normalize_ascii_spaces", "name"), ("tabs_to_ascii_space", "name"), ("normalize_unicode_whitespace", "name")}
    ...
    for before, after in zip(original, preview):
        ...
        if operation == "remove_ascii_phone_separators": after[column] = before[column].replace(" ", "").replace("-", "")
        elif operation == "normalize_ascii_spaces": after[column] = re.sub(r" +", " ", before[column])
        elif operation == "tabs_to_ascii_space": after[column] = before[column].replace("\t", " ")
        else: after[column] = before[column].replace("\u00a0", " ").replace("\u202f", " ")
    ...
    def expected(value):
        if operation == "remove_ascii_phone_separators": return value.replace(" ", "").replace("-", "")
        if operation == "normalize_ascii_spaces": return re.sub(r" +", " ", value)
        if operation == "tabs_to_ascii_space": return value.replace("\t", " ")
