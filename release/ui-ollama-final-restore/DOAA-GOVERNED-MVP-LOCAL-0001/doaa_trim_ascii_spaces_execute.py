...
def execute(payload):
    ...
    if preview_hash is not None and (not isinstance(preview_hash, str) or len(preview_hash) != 64 or preview_hash != sha(inp.read_bytes())):
        return blocked("preview_source_hash_mismatch")
