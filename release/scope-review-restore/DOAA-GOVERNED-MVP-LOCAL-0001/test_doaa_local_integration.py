...
nested_command = m.classify({"request_id":"req-9","payload":{"meta":{"command":"dir"}},"execution_authority":"none"})
assert nested_command["status"] == "integration_blocked"
deep = "x"
for _ in range(m.MAX_DEPTH + 1):
    deep = {"nested": deep}
assert m.classify({"request_id":"req-10","payload":deep,"execution_authority":"none"})["status"] == "integration_blocked"
large = m.classify({"request_id":"req-11","payload":{"text":"x" * (m.MAX_PAYLOAD_BYTES + 1)},"execution_authority":"none"})
assert large["reason"] == "payload_too_large"
unsupported = m.classify({"request_id":"req-12","payload":{"value":{1, 2}},"execution_authority":"none"})
assert unsupported["reason"] == "payload_json_invalid"
