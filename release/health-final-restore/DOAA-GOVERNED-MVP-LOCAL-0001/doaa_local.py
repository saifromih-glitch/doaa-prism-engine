...
from doaa_unicode_csv_execute import execute as execute_unicode_csv
from doaa_unicode_xlsx_execute import execute as execute_unicode_xlsx


COMPLETED_STATUSES = {
    "executed_safe_file",
    "excel_executed_safe_file",
    "space_normalize_executed_safe_file",
    "normalize_unicode_whitespace_executed_safe_file",
    "normalize_unicode_xlsx_executed_safe_file",
}
...
def main():
    ...
    if operation == "normalize_unicode_whitespace" and suffix == ".csv":
        result = execute_unicode_csv(payload)
    elif operation == "normalize_unicode_whitespace" and suffix == ".xlsx":
        result = execute_unicode_xlsx(payload)
    elif operation == "tabs_to_ascii_space":
    ...
    print(json.dumps({"status": "local_flow_completed" if result.get("status") in COMPLETED_STATUSES else "local_flow_blocked", "diagnostics": diagnostics, "execution": result, "execution_started": result.get("execution_started", False)}, ensure_ascii=False, sort_keys=True, separators=(",", ":")))
