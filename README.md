...
`doaa_learning_registry.py` records explicit, consented experiences and turns them into candidates without changing the core source or Dody governance. `doaa_learning_loop.py` automates observation and candidate generation only; it never promotes a candidate. A candidate can become active only after deterministic validation, benchmark and safety receipts, and explicit human approval. `doaa_learning_evaluator.py` requires token reduction, a minimum quality score, and a passing safety signal; active records can be revoked for rollback. This is continuous improvement, not unrestricted self-modification. See `CONTRACT-DOAA-LEARNING-0001.json`, `ADR-DOAA-LEARNING-0001.md`, and the learning tests.
