...
OUTPUT = Path("benchmark-data/arabicaqa/manus-checkpoint-run.json")
...
def call(messages: list[dict], response_format: dict | None = None) -> dict:
    ...
    kwargs = {"model": MODEL, "messages": messages, "max_completion_tokens": 256}
    if response_format:
...
session = WarmCheckpointSession("manus-arabicaqa-warm")
registered = session.register_source(payloads[0]["context"])
if registered["status"] != "source_registered":
    raise RuntimeError("checkpoint_registration_failed")
compact_queries = []
for index, payload in enumerate(payloads):
    prepared = session.prepare_query(payload["question"])
    if prepared["status"] != "warm_query_ready":
        raise RuntimeError("checkpoint_query_failed")
    compact_queries.append({"i": index, "p": prepared["payload"]})
compact = {"v": 1, "ck": registered["checkpoint_id"], "src": payloads[0]["context"], "q": compact_queries}
doaa_system = "doaa.alg.v1|ctx=السياق|q=الأسئلة. لكل i أخرج أقصر إجابة صحيحة مدعومة حرفياً من ctx، دون شرح أو معلومات إضافية. أخرج JSON فقط، واجعل كل إجابة لا تتجاوز 20 كلمة وتحتوي الحد الأدنى الذي يجيب السؤال من ctx."
...
doaa_rows = [{"case_id": case["case_id"], "reference_answer": case["reference_answer"], "answer": answers.get(index, ""), "reference_overlap": overlap(answers.get(index, ""), case["reference_answer"]), "expanded_reference": session.expand_for_verification(compact_queries[index]["p"])} for index, case in enumerate(selected)]
